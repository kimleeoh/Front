...
// LogoFluid Component
const LogoFluid = ({ totalSteps, currentStep, color, size = 300, svgPath }) => {
  const canvasRef = useRef(null);
  const contextRef = useRef(null);
  const animationFrameId = useRef(null);
  const targetFillPercentage = useRef(clamp((currentStep / totalSteps) * 100, 0, 100));
  const currentFillPercentage = useRef(targetFillPercentage.current);
  const clippingPath = useRef(null); // To store Path2D for SVG clipping

  // Update the target percentage and current fill percentage when props change
  useEffect(() => {
    targetFillPercentage.current = clamp((currentStep / totalSteps) * 100, 0, 100);
  ...
  const setupCanvas = () => {
    ...
    // Load and create clipping path from SVG
    if (svgPath) {
      const path = new Path2D(svgPath);
      clippingPath.current = path;
    }
  };
  ...
  const draw = (timestamp) => {
    ...
    if (Math.abs(currentFillPercentage.current - targetFillPercentage.current) > 0.1) {
      const speed = 2; // Adjust this value for smoother or faster transitions
      currentFillPercentage.current += (targetFillPercentage.current - currentFillPercentage.current) * speed * dt;
      currentFillPercentage.current = clamp(currentFillPercentage.current, 0, 100);
    }
  ...
  const drawFluidLayer = (context, layer, dt) => {
    ...
    layer.horizontalPosition = (layer.horizontalPosition + layer.horizontalSpeed * dt) % (Math.pow(2, 53));
    ...
    const fluidAmount = currentFillPercentage.current * (getMeterRadius() - options.borderWidth * 2) / 100;
    ...
    for (let x = 0; x < options.size; x++) {
      const y = layer.initialHeight + amplitude * Math.sin((x + layer.horizontalPosition) / layer.frequency);
      context.lineTo(x, y);
    }
  ...
  const drawPercentageText = (context) => {
    const text = options.drawPercentageSign ? `${currentFillPercentage.current.toFixed(0)}%` : `${currentFillPercentage.current.toFixed(0)}`;
  ...
  }, [options.size, options.foregroundColor, svgPath]);
...
LogoFluid.propTypes = {
  ...
  svgPath: PropTypes.string.isRequired, // SVG 경로를 받도록 설정
};

LogoFluid.defaultProps = {
  color: '#495EF6',
  size: 300,
    svgPath: './logo_main.svg', // 기본값으로 로고 SVG 파일 경로 설정
    totalSteps: 100,
    currentStep: 50,
};
